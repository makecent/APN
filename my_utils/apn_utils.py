...
def apn_detection_on_single_video(progressions, det_kwargs=None):
    sampling = det_kwargs.get('sampling', 1000)
    ...
    scale_ratio = original_len / sampling
    ...
    for class_ind, progs_by_class in enumerate(progressions.T):
        ...
        dets_by_class *= scale_ratio
        dets_by_class, keep = nms1d(dets_by_class, scores_by_class, **nms_kwargs)
        scores_by_class = scores_by_class[keep]

        dets.append(dets_by_class)
        scores.append(scores_by_class)
...
#     return remained
def nms1d(dets, scores, iou_thr=0.0, top_k=np.inf, return_idx=False, backend='torch'):
    """NMS for temporal proposals.
...
def one_vs_n_iou(one_box, n_boxes, backend='torch'):
