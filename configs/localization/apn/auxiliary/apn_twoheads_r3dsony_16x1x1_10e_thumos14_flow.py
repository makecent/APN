...
model = dict(
    ...
    cls_head=dict(
        ...
        in_channels=1024,
        output_type='two_heads',
        loss=dict(type='ApnTwoLoss'),
        spatial_type='avg3d',
        clip_len=clip_len))
...
# evaluation
evaluation = dict(interval=1, save_best='mae', metrics=['mae', 'framecls_top_k_accuracy_based_on_apn', 'framecls_mean_class_accuracy_based_on_apn'], dataset_name='Val')

# others
checkpoint_config = dict(interval=1)
log_config = dict(interval=1000, hooks=[dict(type='TensorboardLoggerHook'), dict(type='TextLoggerHook')])

# runtime settings
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './work_dirs/apn_twoheads_r3dsony_16x1x1_10e_thumos14_flow/'
load_from = None
resume_from = None
workflow = [('train', 1)]
