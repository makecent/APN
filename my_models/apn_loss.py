...
from torch.nn.functional import binary_cross_entropy, l1_loss, nll_loss, cross_entropy, softmin, kl_div
import torch
from torch import sigmoid, softmax, clamp, log_softmax, tensor, argmax, zeros, sqrt, arange
import numpy as np
...
class ApnMAELoss(ApnBaseLoss):
    ...
    def _forward_zeros(self, output, progression_label, class_label):
        ...
        batch_loss = 0
        for output_e, target_progression, target_class in zip(output, progression_label, class_label):
    ...
    def _forward_random(self, output, progression_label, class_label):
        ...
        uncorrelated_loss = l1_loss(uncorrelated_progs_sum, random_sum_target)/((num_action-1) * num_sample)
...
class ApnCELoss(ApnBaseLoss):
    ...
    def _forward_random(self, output, progression_label, class_label):
        ...
        correlated_loss = self._forward_ignore(output, progression_label, class_label)
        correlated_prog_sum = self.val_output(output, class_label, return_tensor=True).sum()
        uncorrelated_progs_sum = self.test_output(output, return_tensor=True).sum() - correlated_prog_sum
        random_sum_target = tensor((num_action - 1) * num_sample * ((num_stage-1) / 2))
        uncorrelated_loss = l1_loss(uncorrelated_progs_sum, random_sum_target)/tensor((num_action - 1) * num_sample * (num_stage-1))
    ...
    def test_output(output, return_tensor=False):
        ...
        stage_cuboid = arange(0, num_stage, device=output.device).unsqueeze(0).unsqueeze(0).expand(num_sample,
                                                                                                         num_action,
                                                                                                         num_stage)
...
class ApnBCELoss(ApnBaseLoss):
    ...
    def _forward_zeros(self, output, progression_label, class_label):
        ...
        num_stage = output.shape[2]
        batch_loss = 0
        for output_e, target_progression, target_class in zip(output, progression_label.long(), class_label):
            target_output = zeros(num_action, num_stage, dtype=torch.int64).cuda()
            target_output[target_class, :] = target_progression
            loss_e = cross_entropy(output_e.view(-1, 2), target_output.view(-1), reduction='none').view(num_action,
                                                                                                          num_stage).mean(
                dim=-1)
            loss_e[target_class] *= (num_action - 1)
            batch_loss += loss_e.sum() / (num_action - 1)
    ...
    def _forward_random(self, output, progression_label, class_label):
        ...
        for output_e, target_progression, target_class in zip(output, progression_label, class_label):
            ...
            correlated_loss = cross_entropy(correlated_output, target_progression.long())
            correlated_prog_sum = sigmoid((softmax(correlated_output, -1)[:, 1]-0.5)*10).sum() ## mimic the step function: >0.5 then 1 otherwise 0
            uncorrelated_progs_sum = sigmoid((softmax(output_e, -1)[:, :, 1]-0.5)*10).sum() - correlated_prog_sum
        ...
        random_sum_target = tensor((num_action - 1) * num_sample * (num_stage / 2))
        uncorrelated_loss = l1_loss(batch_uncorrelated_progs_sum, random_sum_target)
        uncorrelated_loss = uncorrelated_loss /tensor((num_action - 1) * num_sample * num_stage)
        loss = correlated_loss + 1 * uncorrelated_loss
        return loss
...
class ApnSORDLoss(ApnCELoss):
    ...
    def _forward_zeros(self, output, progression_label, class_label):
        ...
        stage_matrix = arange(0, num_stage, device=progression_label.device).unsqueeze(0).expand(num_action,
                                                                                                       num_stage)
        batch_loss = 0
        for output_e, target_progression, target_class in zip(output, progression_label, class_label.squeeze(-1)):
            target_output = zeros(num_action, 1).cuda()
            target_output[target_class] = target_progression
            distance = sqrt(abs(stage_matrix - target_output).float())
            smoothed_label = softmin(distance, dim=-1)

            loss_e = kl_div(output_e, smoothed_label, reduction='none').sum(dim=-1)
            loss_e[target_class] *= (num_action - 1)
            batch_loss += loss_e.sum() / (num_action - 1)
        loss = batch_loss / num_sample
        return loss
...
class ApnCORALLoss(ApnBaseLoss):
    ...
    def _forward_zeros(self, output, progression_label, class_label):
        ...
        for output_e, target_progression, target_class in zip(output, progression_label, class_label):
            ...
            target_output[target_class, :] = target_progression
            loss_e = binary_cross_entropy(output_e, target_output, reduction='none').mean(dim=-1)
            loss_e[target_class] *= (num_action - 1)
            batch_loss += loss_e.sum() / (num_action - 1)
        loss = batch_loss / num_sample
        return loss
    ...
    def _forward_random(self, output, progression_label, class_label):
        ...
        for output_e, target_progression, target_class in zip(output, progression_label, class_label):
            correlated_output = output_e[target_class.squeeze()]
            correlated_loss = binary_cross_entropy(correlated_output, target_progression)
            correlated_prog = sigmoid((correlated_output-0.5)*10).sum() ## mimic the step function: >0.5 then 1 otherwise 0
            uncorrelated_progs_sum = sigmoid((output_e-0.5)*10).sum() - correlated_prog
        ...
        correlated_loss = batch_correlated_loss / num_sample
        random_sum_target = tensor((num_action - 1) * num_sample * (num_stage / 2))
        uncorrelated_loss = l1_loss(batch_uncorrelated_progs_sum, random_sum_target)/random_sum_target
        loss = correlated_loss + 1 * uncorrelated_loss
        return loss
    ...
    def test_output(output):
        ...
        progressions = np.count_nonzero(output > 0.5, axis=-1)
        progressions = progressions * 100 / num_stage
        return progressions
...
class ApnTwoLoss(BaseWeightedLoss):
    ...
    def test_output(self, output):
        ...
        stage_matrix = arange(0, num_stage, device=prog.device).unsqueeze(0).expand(num_sample, num_stage)
